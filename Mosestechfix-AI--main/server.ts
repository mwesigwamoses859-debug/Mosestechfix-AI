...
function signingSecret() {
  ...
  if (!secret || secret.length < 32) throw new Error('ACCESS_TOKEN_SECRET must contain at least 32 characters.');
  return secret;
}
...
function verifySignedPayload(code: string): any {
  const [encoded, suppliedSignature] = code.trim().split('.');
  if (!encoded || !suppliedSignature) throw new Error('Invalid activation-code format.');
...
function adminSecretMatches(value: unknown) {
  const expectedValue = process.env.ADMIN_ACTIVATION_SECRET;
...
app.post('/api/subscriptions/manual/generate-code', (req, res) => {
  try {
    if (!adminSecretMatches(req.header('x-admin-secret'))) return res.status(401).json({ error: 'Administrator password is incorrect.' });
    const { planId, customerName, customerPhone, transactionReference, deviceId } = req.body || {};
    const plan = MANUAL_PLANS[planId];
    if (!plan) return res.status(400).json({ error: 'Invalid subscription plan.' });
    if (![customerName, customerPhone, transactionReference, deviceId].every((value) => typeof value === 'string' && value.trim())) {
      return res.status(400).json({ error: 'Customer, phone, transaction reference and device code are required.' });
    }
    const expiresAt = new Date(Date.now() + plan.durationDays * 86_400_000);
    const activationCode = signPayload({
      version: 1,
      planId,
      ...
      amountUGX: plan.amountUGX,
      issuedAt: new Date().toISOString(),
      expiresAt: expiresAt.toISOString(),
      nonce: crypto.randomUUID(),
    });
    res.json({ activationCode, planName: plan.name, expiresAt: expiresAt.toISOString() });
  } catch (error: any) {
    console.error('Manual Code Generation Error:', error.message);
    res.status(500).json({ error: error.message || 'Could not generate the activation code.' });
  }
});

app.post('/api/subscriptions/manual/activate', (req, res) => {
  try {
    const { activationCode, deviceId } = req.body || {};
    if (typeof activationCode !== 'string' || typeof deviceId !== 'string') return res.status(400).json({ error: 'Activation code and device code are required.' });
    const payload = verifySignedPayload(activationCode);
    const plan = MANUAL_PLANS[payload.planId];
    if (!plan || payload.deviceId !== deviceId.trim()) return res.status(403).json({ error: 'This activation code belongs to a different device.' });
    const expiresAt = new Date(payload.expiresAt);
    if (!Number.isFinite(expiresAt.getTime()) || expiresAt <= new Date()) return res.status(403).json({ error: 'This activation code has expired.' });
    res.json({ accessToken: activationCode, planName: plan.name, expiresAt: expiresAt.toISOString() });
  } catch (error: any) {
